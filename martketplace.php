...
class OpenSimMarketplace {
    ...
    public function settings_page(): void {
        ...
        if (isset($_POST['submit']) && check_admin_referer('osmp_settings', '_wpnonce')) {
            ...
            if (!empty($_POST['osmp_os_db_pass']))    update_option('osmp_os_db_pass', $this->encrypt_option(sanitize_text_field($_POST['osmp_os_db_pass'])));
            if (!empty($_POST['osmp_money_db_pass'])) update_option('osmp_money_db_pass', $this->encrypt_option(sanitize_text_field($_POST['osmp_money_db_pass'])));
    ...
    public function sync_prims(int $batch_size = 500): void {
        if (!$this->os_db) { error_log('OpenSim Marketplace: OS database not connected for sync_prims'); return; }
        if (!$this->is_valid_uuid($this->region_uuid)) { error_log('OpenSim Marketplace: Region UUID is missing/invalid, sync aborted'); return; }
        ...
        $last_offset = (int) get_option('osmp_last_offset', 0);

        try {
            $sql = "SELECT UUID, Name FROM prims WHERE RegionUUID = ? LIMIT {$last_offset}, {$batch_size}";
            $stmt = $this->os_db->prepare($sql);
            if (!$stmt) { error_log('OpenSim Marketplace: Failed to prepare sync_prims query'); return; }

            $stmt->bind_param('s', $this->region_uuid);
            if (!$stmt->execute()) { error_log('OpenSim Marketplace: Failed to execute sync_prims query'); $stmt->close(); return; }
            ...
            while ($prim = $result->fetch_assoc()) {
                ...
                if (in_array($uuid_l, $legacy_ignore, true) || in_array($uuid_l, $uuid_rules, true)) continue;
                if (!empty($name_exact) && in_array($name_l, $name_exact, true)) continue;
                if (!empty($name_contains)) {
                    $skip = false;
                    foreach ($name_contains as $needle) { if ($needle !== '' && strpos($name_l, $needle) !== false) { $skip = true; break; } }
                    if ($skip) continue;
                }
                ...
                if (empty($existing)) {
                    ...
                    if ($post_id && !is_wp_error($post_id)) {
                        ...
                        update_post_meta($post_id, '_region_uuid', $this->region_uuid);
            ...
            $stmt->close();
            update_option('osmp_last_offset', $last_offset + $batch_size);

            if ($count < $batch_size) update_option('osmp_last_offset', 0);
            else wp_schedule_single_event(time() + 1, 'osmp_cron_import');

            error_log("OpenSim Marketplace: sync_prims processed {$count} items (offset {$last_offset})");
        } catch (Throwable $e) {
            error_log('OpenSim Marketplace sync_prims error: ' . $e->getMessage());
        }
    }
